...
def obj_func(system):
    ...
    res = minimize(ret_system.modelFunction, x0, method='Nelder-Mead', tol=1e-6)
...
def moveSystem(system):
    ...
    for i in range(1,10):
        ...
        x0_orient = np.append(x0_orient, system.nodes[i].orient)
    x0 = np.append(x0_pos, x0_orient)
    res = minimize(system.modelFunction, x0, method='Nelder-Mead', tol=1e-6)
...
best = annealer.optimize(max_iters=10, inner_iters=10, alpha=0.99)
moveSystem(best[0])
#
for i in range(0, 3):
